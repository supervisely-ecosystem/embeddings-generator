...
@to_thread
def remove_collections(collection: sly.EntitiesCollectionInfo, progress: tqdm = None):
...
def main():
    project_info = sly.run_coroutine(get_project_info(api, 426))
    ...
    tasks = []
    tasks.append(get_collections(project_info, collections))
    ...
    if len(collections) == 0:
        ...
        if len(tasks) > 0:
            sly.run_coroutine(asyncio.gather(*tasks))
            sly.logger.info("All collections removed.")

    tasks = []
    tasks.append(clean_image_embeddings_updated_at(project_info))
    tasks.append(set_embeddings_in_progress(api, project_info.id, False))
    if len(tasks) > 0:
        sly.run_coroutine(asyncio.gather(*tasks))

    tasks = []
    tasks.append(switch_off_auto_update(project_info))

    if len(tasks) > 0:
        sly.run_coroutine(asyncio.gather(*tasks))
    sly.logger.info("All operations completed.")
